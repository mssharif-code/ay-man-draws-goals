...
const faqs = [
  { q: "How are helpers verified?", a: "Every helper completes an identity check and a background screening before being listed. We also collect references from past employers." },
  { q: "What if I need to cancel a booking?", a: "You can cancel up to 4 hours before the scheduled time at no charge. Later cancellations may be charged for the first hour." },
  { q: "How do I pay the helper?", a: "Payment is handled securely through the app after each shift. You can use UPI, cards, or net banking. Helpers are paid within 24 hours." },
  { q: "Can I book the same helper again?", a: "Yes. Once you find a helper you like, you can mark them as a favourite and rebook them with one tap whenever you need." },
  { q: "What areas do you cover?", a: "We currently serve major neighbourhoods across Mumbai, with new areas added every month. Check each helper's profile for their location." },
];

const plans = [
  { name: "Pay-as-you-go", price: "₹0", period: "/booking", features: ["No monthly fee", "Book any helper", "Standard support", "Cancel up to 4h before"], featured: false },
  { name: "Haven Plus", price: "₹499", period: "/month", features: ["10% off every booking", "Priority matching", "Dedicated support", "Free rescheduling"], featured: true },
];
    ...
    <main>
      <section className="hero">
        <div className="container">
          ...
          <a className="hero-cta" href="#helpers">Find a helper</a>
        </div>
      </section>
      ...
      <section id="helpers" className="section">
        <div className="container">
          ...
          <p className="section-sub">Each helper is background-checked and reviewed by families like yours.</p>
            ...
            {helpers.map((h) => (
              <article key={h.id} className="helper-card">
                ...
                <div className="helper-body">
                  ...
                  <p className="helper-rate"><strong>₹{h.rate}</strong> / hour</p>
      ...
      <section id="responsibilities" className="section section-alt">
        <div className="container">
          ...
          <p className="section-sub">Know exactly what each helper takes care of before you book.</p>
      ...
      <section id="records" className="section section-alt">
        <div className="container">
          ...
          <p className="section-sub">Every booking you make is saved here with your name and the time.</p>
          {loadingRecords ? (
            <div className="records-empty">Loading records…</div>
          ) : records.length === 0 ? (
            <div className="records-empty">No bookings yet. Book a helper above and it will appear here.</div>
          ) : (
            <div>
              {records.map((r) => (
              ...
              <button className="refresh-btn" onClick={onReload}>Refresh records</button>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="modal-body">
          {done ? (
            <div className="success">
              ...
              <h3>Booking saved!</h3>
              <p>{helper.name} will be notified. Your record has been saved below.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="field">
                <label htmlFor="bk-name">Your name</label>
                <input id="bk-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Priya Sharma" />
                {errors.name && <p className="field-error">{errors.name}</p>}
              </div>
              <div className="field">
                <label htmlFor="bk-service">Service</label>
                <select id="bk-service" value={service} onChange={(e) => setService(e.target.value)}>
              ...
              <div className="field">
                ...
                <input id="bk-hours" type="number" min={1} max={12} value={hours} onChange={(e) => setHours(Number(e.target.value))} />
                {errors.hours && <p className="field-error">{errors.hours}</p>}
              </div>
              <div className="summary">
                <div className="summary-row"><span>Rate</span><span>₹{helper.rate}/hour</span></div>
                <div className="summary-row"><span>Hours</span><span>{hours}</span></div>
              ...
              <button className="btn-submit" type="submit" disabled={submitting}>
                {submitting ? "Saving…" : `Confirm & pay · ₹${total.toLocaleString("en-IN")}`}
              </button>
            </form>
          )}
        </div>
